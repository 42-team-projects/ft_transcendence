...
async def remove_from_queue(players):
    ...
    if players[0] in queue:
        queue.remove(players[0])
    if players[1] in queue:
...
class MatchMaikingConsumer(AsyncWebsocketConsumer):
    ...
    async def wait_for_opponent(self):
        ...
        if len(queue) >= 2:
            ...
            room_group_name = f"game_{player_2.id}-{player_1.id}"
            message = {
                'text': 'Opponent found',
                'room_group_name': room_group_name,
            }
            await self.send_message(message=message, room_group_name=room_group_name)

    async def send_message(self, message, function='game_message', room_group_name=None):
        await self.channel_layer.group_send(
            room_group_name,
            {
                'type': function,
